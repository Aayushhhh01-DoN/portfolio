      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {projects.map((project, index) => (
            <div
              key={index}
              className="card-cyber rounded-lg p-6 group hover-lift transition-all duration-500 glow-border"
            >
              {/* Header */}
              ...
              <div className="flex flex-wrap gap-1.5 mb-5">
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-2 py-1 text-[10px] font-mono rounded bg-secondary/50 text-muted-foreground border border-border/30 hover:border-primary/30 hover:text-primary transition-all"
                  >
                    {tag}
                  </span>
                ))}
