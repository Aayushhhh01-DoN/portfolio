          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {skillCategories[activeTab].skills.map((skill, skillIndex) => {
              ...
              return (
                <div
                  key={skill.name}
                  className={`card-cyber rounded-lg p-5 text-center hover-lift group cursor-default glow-border transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
                  style={{ transitionDelay: `${skillIndex * 80 + 300}ms` }}
                >
                  <Icon
                    className="w-8 h-8 mx-auto mb-3 transition-transform duration-300 group-hover:scale-125"
                    style={{ color: skill.color }}
                  />
                  <span className="text-sm font-medium text-foreground group-hover:text-primary transition-all relative z-10">
                    {skill.name}
                  </span>
                </div>
              );
          ...
          {skillCategories.flatMap(c => c.skills).map((skill) => {
            ...
            return (
              <Badge
                key={skill.name}
                variant="outline"
                className="border-border/50 text-muted-foreground/70 text-[10px] font-mono hover:border-primary/30 hover:text-primary hover:bg-primary/5 transition-all cursor-default gap-1.5 py-1"
              >
                <Icon className="w-3 h-3" style={{ color: skill.color }} />
                {skill.name}
              </Badge>
            );
          })}
