        ...
        <div className="relative">
          ...
          <div className="space-y-6">
            {experiences.map((exp, index) => (
              <div
                key={index}
                className={`relative flex gap-6 group transition-all duration-600 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"}`}
              ...
              >
                ...
                <div className="flex-1 card-cyber rounded-lg p-6 hover-lift glow-border">
                  <div className="flex flex-wrap items-center gap-3 mb-2">
                    <span className="text-xs font-mono text-primary px-2 py-1 rounded bg-primary/5 border border-primary/20 neon-box">{exp.period}</span>
                    {index === 0 && (
                  ...
                  <p className="text-sm text-accent mb-3 flex items-center gap-1"><ChevronRight className="w-3 h-3" />{exp.company}</p>
